...
type DepartmentQuery struct {
	...
	// eager-loading edges.
	withDepartmentDiagnose *DiagnoseQuery
...
// QueryDepartmentDiagnose chains the current query on the department_diagnose edge.
func (dq *DepartmentQuery) QueryDepartmentDiagnose() *DiagnoseQuery {
...
//  WithDepartmentDiagnose tells the query-builder to eager-loads the nodes that are connected to
// the "department_diagnose" edge. The optional arguments used to configure the query builder of the edge.
func (dq *DepartmentQuery) WithDepartmentDiagnose(opts ...func(*DiagnoseQuery)) *DepartmentQuery {
	query := &DiagnoseQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDepartmentDiagnose = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withDepartmentDiagnose; query != nil {
		...
		query.Where(predicate.Diagnose(func(s *sql.Selector) {
			s.Where(sql.InValues(department.DepartmentDiagnoseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.DepartmentDiagnose = append(node.Edges.DepartmentDiagnose, n)
		}
	}
